...
void *emalloc(size_t s){
	...
	if (NULL == result) {
		fprintf(stderr, "there was a malloc error");
		exit(EXIT_FAILURE);
	}
	return result;
}
...
int main(void){
	...
	while (count < array_size && 1 == scanf("%s", word)){
		word_array[count] = emalloc((strlen(word)+1) *sizeof word_array);
...
/* gcc -W -Wall -O2 -ansi - pedantic -g prac1.c -o prac1 */ //how to compile